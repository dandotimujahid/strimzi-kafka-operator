...
function install_helm_unittest {
    ...
    if [ $ARCH != "s390x"]; then
        ...
        #building helm unittest from source
        echo "building helm unittest from source"
        git clone https://github.com/helm-unittest/helm-unittest
        cd helm-unittest/
        git checkout $TEST_HELM_UNITTEST_VERSION
        cd cmd/helm-unittest
        go build -o helm-unittest .
        sudo cp helm-unittest /usr/local/bin/
        # run a proper helm command instead of, for example, "which helm", to verify that we can call the binary
        echo "Verifying the installation of helm unittest plugin ..."
        helm-unittest --help
        helmCommandOutput=$?    
